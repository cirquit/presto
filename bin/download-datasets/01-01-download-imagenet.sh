...
#echo "---"
#echo "Starting docker container to download the https://www.kaggle.com/competitions/imagenet-object-localization-challenge/data dataset"
...
URL="https://syncandshare.lrz.de/dl/fiWHsi4sgHC24UGEAwUYFC/imagenet-small-isenko-v2.zip"

wget -q --show-progress ${URL} --no-check-certificate -P ${IMAGENET_DATASET_PATH}

cd ${IMAGENET_DATASET_PATH}
unzip imagenet-small-isenko-v2.zip
echo "${bold}-- Finished extraction!"


echo "${bold}1.1.4 Removing compressed files...${normal}"
rm imagenet-small-isenko-v2.zip
